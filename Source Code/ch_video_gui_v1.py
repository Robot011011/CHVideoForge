"""
Clone Hero Video Helper - PySide6 GUI
-------------------------------------
- Download YouTube video as Clone Hero–ready WebM (VP8)
- Optional embedded audio (Vorbis) in the same WebM
- Trim & pad (black) support
...
def run_ffmpeg_with_progress(
    ...
    Normal progress/status noise like 'progress=continue' is hidden from
    the status label, but still sent to debug_cb.
    """
...
def download_video(
    ...
    """
    Download best video-only (default) or best video+audio stream <=1080p.
    - include_audio = False → video-only
    - include_audio = True  → video+audio in one file
    If COOKIES_FILE is set, yt-dlp will use that cookies.txt for
    authentication / anti-bot checks.
    """
    if include_audio:
...
def convert_to_clonehero_webm(
    ...
    - If keep_audio=True  → add/re-encode audio (libvorbis)
...
class Worker(QObject):
    ...
    def _run_download(self) -> Path:
        ...
        # Base temp path (without caring too much about final extension)
        ...
        # 1) Download (yt-dlp might still tweak the extension internally)
        download_video(
            url,
            tmp_file,
            include_audio=keep_audio,
            progress_cb=self.progress.emit,
            status_cb=self.status.emit,
            debug_cb=self.debug.emit,
        )

        # 2) Resolve the actual file yt-dlp created
        input_file = tmp_file
        ...
        # 3) Convert to Clone Hero format
        convert_to_clonehero_webm(
            input_file,
            output_path,
            trim_start=trim,
...
class MainWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        # Output name
        dl_layout.addWidget(QLabel("Output name (.webm):"), 2, 0)
    ...
    def apply_preset_clone_hero_bg(self):
        ...
        self.status_label.setText("Preset applied: Clone Hero BG (silent video).")
